...
def multilayer_perceptron(x, weights, biases, keep_prob):
    ...
    layer_1 = tf.nn.dropout(layer_1, 1 - (keep_prob))

    layer_2 = tf.add(tf.matmul(layer_1, weights['h2']), biases['b2'])
    layer_2 = tf.nn.sigmoid(layer_2)
    layer_2 = tf.nn.dropout(layer_2, 1 - (keep_prob))
...
def run(trainFile, trainLabelFile, testFile,testLabelFile, groupFile,  suspFile,loss, featureNum, nodeNum):
    ...
    optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost+regularizer)
