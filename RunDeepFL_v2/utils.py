...
def loss_func(pred, y, loss, datasets, groups=[]):
    if loss == 0: # weighted softmax
        ...
        weight_per_label = tf.transpose( a=tf.matmul(y, tf.transpose(a=classes_weights)) )
        xent = tf.multiply(weight_per_label, tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=y)) #shape [1, batch_size]              
        cost = tf.reduce_mean(input_tensor=xent)
    elif loss == 1: # softmax
        cost= tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=y))
        ...
        diff=tf.subtract(pred_1,tf.transpose(a=pred_1))
        grouping=tf.equal(groups, tf.transpose(a=groups))
        grouping=tf.cast(grouping,tf.float32)
        mask=y_1*tf.transpose(a=1-y_1)*grouping
        diff_exp=tf.exp(-diff)
        cost=tf.reduce_sum(input_tensor=mask*diff_exp) 
        ...
        grouping=tf.cast(grouping,tf.float32)
        mask=y_1*tf.transpose(a=1-y_1)*grouping
        pair_num=tf.reduce_sum(input_tensor=mask)
        diff_exp=tf.exp(-diff)
        softmax_cost=tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=y))
        cost= tf.cond(pred=tf.greater(pair_num,tf.constant(0.0)), true_fn=lambda: softmax_cost+tf.divide(tf.reduce_sum(input_tensor=mask*diff_exp),pair_num), false_fn=lambda: softmax_cost)
        ...
        mask=y_1*tf.transpose(a=1-y_1)*grouping
        diff=mask*diff
        cost=tf.reduce_sum(input_tensor=tf.subtract(mask,diff)) 
        ...
        diff=mask*diff
        softmax_cost=tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=y))
        cost= tf.cond(pred=tf.greater(pair_num,tf.constant(0.0)), true_fn=lambda: softmax_cost+tf.reduce_sum(input_tensor=tf.divide(tf.subtract(mask, diff),pair_num)), false_fn=lambda: softmax_cost)
        ...
        pred_x=tf.boolean_mask(tensor=pred_1,mask=positive)
        pred_x=tf.reshape(pred_x,[-1,1])
        pred_y=tf.boolean_mask(tensor=pred_1,mask=negative)
        pred_y=tf.reshape(pred_y,[1,-1])
        diff=tf.subtract(pred_x,pred_y)
        diff_exp=tf.exp(-diff)
        cost=tf.reduce_sum(input_tensor=diff_exp) 
    return cost
