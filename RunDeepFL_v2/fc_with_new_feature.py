...
def new_model(spec, new_features,m1,m2,m3,m4,complexity,similarity, keep_prob,is_training):
    model_size_times = 2
    with tf.compat.v1.variable_scope('seperate_mut',reuse=False):
        ...
        with tf.compat.v1.variable_scope('mut_concat',reuse=False):
            mut_concat = single_fc_layer(mut_concat,35*4*model_size_times,35*model_size_times, keep_prob,is_training)

    with tf.compat.v1.variable_scope('seperate_spec',reuse=False):
        ...
        with tf.compat.v1.variable_scope('new',reuse=False):
            #spec = tf.layers.batch_normalization(spec, training=is_training)
...
def run(trainFile, trainLabelFile, testFile,testLabelFile, groupFile, suspFile,loss, featureNum, nodeNum):
    ...
    # tf Graph input
    x = tf.compat.v1.placeholder("float", [None, 259])
    spec = tf.compat.v1.placeholder("float", [None, 34])
    new_features = tf.compat.v1.placeholder("float", [None, 33])
